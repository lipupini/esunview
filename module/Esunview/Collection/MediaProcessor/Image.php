...
namespace Module\Esunview\Collection\MediaProcessor;
...
class Image extends \Module\Lipupini\Collection\MediaProcessor\Image {
	...
	public static function processAndCache(State $systemState, string $collectionName, string $fileTypeFolder, string $sizePreset, string $filePath, bool $echoStatus = false): false|string {
		...
		$imagine = (new Imagine\Filter\Basic\Autorotate())->apply(static::imagine()->open($collectionPath . '/' . $filePath));
		// Strip all EXIF data
		$imagine = $imagine->strip();

		// Resize
		$imagine = $imagine->thumbnail(
			new Imagine\Image\Box(
				$systemState->mediaSize[$sizePreset][0],
				$systemState->mediaSize[$sizePreset][1]
			), $sizePreset === 'thumbnail' ? Imagine\Image\ImageInterface::THUMBNAIL_OUTBOUND : Imagine\Image\ImageInterface::THUMBNAIL_INSET)
		;
		...
		if ($sizePreset === 'watermark' && $hasWatermark) {
			...
			$watermark = static::imagine()->open($watermarkImageFile)->thumbnail(
				new Imagine\Image\Box($wSize, $wSize),
				Imagine\Image\ImageInterface::THUMBNAIL_FLAG_UPSCALE
			);
			$bottomRight = new Imagine\Image\Point($size->getWidth() - $wSize, $size->getHeight() - $wSize);
			$imagine = $imagine->paste($watermark, $bottomRight);
		}

		$imagine->save($fileCachePath, $systemState->imageQuality);
